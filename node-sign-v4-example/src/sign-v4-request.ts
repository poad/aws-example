...
async function sign({
  ...
  return signer.sign(new HttpRequest(options));
}
...
async function signRequest(params: SignV4RequestParams) {
  ...
  const port =
    params.endpoint.port && !Number.isNaN(params.endpoint.port)
      ? Number.parseInt(params.endpoint.port)
      : params.endpoint.protocol === 'https:'
        ? 443
        : params.endpoint.protocol === 'http:'
          ? 80
          : undefined;
