...
export class InfraStack extends Stack {
  constructor(scope: Construct, id: string, props: InfraStackProps) {
    ...
    const imageBuildProject = new PipelineProject(this, 'ImageBuildCodeBuildProject', {
      projectName: `${dockerImageName}-codebuild-docker-build-${this.account}`,
      environment: {
      ...
      buildSpec: BuildSpec.fromSourceFilename('codepipeline/codebuild-custom-image/image-build/buildspec.yml'),
    ...
    const appBuildCodeBuildProject = new PipelineProject(this, 'AppBuildCodeBuildProject', {
      projectName: `${dockerImageName}-app-build-${this.account}`,
      environment: {
        buildImage: LinuxBuildImage.fromEcrRepository(repository),
        computeType: ComputeType.SMALL,
        privileged: true,
        environmentVariables: {
          AWS_DEFAULT_REGION: {
      ...
      buildSpec: BuildSpec.fromSourceFilename('codepipeline/codebuild-custom-image/app-build/buildspec.yml'),
      role: new Role(this, 'AppCodeBuildRole', {
        roleName: `${dockerImageName}-app-build-${this.account}-project-role`,
        assumedBy: new ServicePrincipal('codebuild.amazonaws.com'),
        path: '/',
        inlinePolicies: {
          'CodeBuildAccess': new PolicyDocument({
    ...
    new Pipeline(this, 'DockerBuildCodePipeline', {
      ...
      stages: [
        ...
        },
        {
          ...
          actions: [
            new CodeBuildAction({
              project: appBuildCodeBuildProject,
              actionName: 'AddTag',
              input: dockerSourceArtifact,
              runOrder: 1
            }),
